...
import {getOrganizacionById, modifyOrganizacion} from '@/server/api';
...
const RegisterOrganizationPage = () => {
            ...
            api.request(getOrganizacionById, params.id).then(
                async data => {
                    if (data && data.Resultado && data.Resultado[0]) {
                        ...
                        await setCompleteLocation(idProvincia, idCanton, idDistrito);
                    } else {
                        Swal.fire({
                            icon: 'error',
    ...
    const handleRegister = async (data) => {
        ...
        await api.request(modifyOrganizacion, body).then(
            () => {
                Swal.fire({
                    icon: 'success',
